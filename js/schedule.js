...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    document.getElementById('schedule-container').innerHTML = 
      '<p style="text-align: center; color: var(--color-text-secondary);">スケジュールデータの読み込みに失敗しました。</p>';
  }
});
...
function handleSwipe() {
  ...
  if (horizontalDistance < -swipeThreshold) {
    ...
    newDate.setMonth(newDate.getMonth() + 1);
    if (newDate <= maxDate) {
      animateScheduleTransition('left', () => {
        currentDate = newDate;
        renderSchedule(currentDate);
        updateNavigationButtons();
      });
  ...
  if (horizontalDistance > swipeThreshold) {
    ...
    newDate.setMonth(newDate.getMonth() - 1);
    if (newDate >= minDate) {
      animateScheduleTransition('right', () => {
        currentDate = newDate;
        renderSchedule(currentDate);
        updateNavigationButtons();
      });
...
function animateScheduleTransition(direction, onComplete) {
...
function renderSchedule(date) {
  ...
  console.log('Rendering schedule for:', year, month + 1);
  ...
  weekdays.forEach((day, index) => {
    ...
    weekdayCell.className = `schedule-weekday ${weekdayClasses[index]}`;
